 ...
 * will return STN1100_NO_RESPONSE.
 */
#ifndef STN1100_TIMEOUT
    #warning "STN1100_TIMEOUT must be defined."
    #define STN1100_TIMEOUT 9000
#endif
/**
 ...
 */
#ifndef STN1100_BAUD_RATE
    #warning "STN1100_BAUD_RATE must be defined"
    #define STN1100_BAUD_RATE 115200
#endif

/**
 * The serial port/device that the STN1100 chip is connected to.  
 */
#ifndef STN1100_PORT
    #warning "STN1100_PORT is not defined, using default value"
    #define STN1100_PORT Serial1
#endif

/**
 * Return code when communication with the ELM device returned succesfully
 */
#define STN1100_SUCCESS 0
 ...
 *  unexpected output.
 */
#define STN1100_GARBAGE 3


/**
 *  Return code when the ELM device is unable to connect to the vehicle 
 *  The STN1100 has tried all of the available protocols, and could not
 ...
 */
#define STN1100_UNABLE_TO_CONNECT 4
 ...
 */
#define STN1100_NO_DATA 5



/**
 *  Interface for the STN1100 multi-function OBDII Interpreter IC.  Providing
 ...
 *  port must be configured, prior to including the STN1100 header.
 *
 *  The STN1100 class contains all the methods used to interact with the STN1100 device, 
 ...
 *  @code
 *  #define STN1100_TIMEOUT 9000
 *  #define STN1100_BAUD_RATE 9600
 *  #define STN1100_PORT Serial3
 *  #include <STN1100.h>
 *
 *  STN1100 Stn1100;
 *  byte status;
 *  int temperature;
 *
 *  status=Stn1100.begin();
 *  if (status != STN1100_SUCCESS){
 *      Serial.print("STN1100 begin failed with error: ");
 *      Serial.println(status);
 *  }
 *
 *  status=Stn1100.coolantTemperature(temperature);
 *  if (status  == STN1100_SUCCESS){
 ...
 *  to run arbitrary commands directly on the STN1100 device, or request PIDS and
 *  raw commands.
 *
 *  To run an arbitrary command on the STN1100 device, runCommand can be used.  
 *  This will run the given command and wait until the prompt is returned or 
 *  STN1100_TIMEOUT is reached.  The buffer is filled with any data returned from 
 *  the STN1100 controller.
 *
...
class Stn1100
{
    public:
        /**
         *  Configures the STN1100 Device by sending a series of commands:  
         *      - Reset 
         *      - Echo Off
         *      - Auto detect connection.
         *  @return STN1100_SUCCESS when the STN1100 device returned the anticipated
         *  response. Else returns an STN1100 error code.
         */
        byte begin();

        /** 
         *  Issues the ATI command to the STN1100 device.  The ATI command 
         *  causes the chip to identify itself, by printing the startup
         *  product ID string (currently â€˜STN1100 v1.3aâ€™). Software can
        ...
        /** 
         *  Issues the AT RV command to the STN1100 device.  This initiates the
         ...
         *  connected to the voltage to be measured through a 47KÎ© and 10KÎ©
         *  resistor divider (with the 10KÎ© connected from pin 2 to Vss), and that
         *  the STN1100 supply is a nominal 5V. This will allow for the measurement
